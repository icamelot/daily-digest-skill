...
当收到回调 `mail:mark-normal-read` 或用户说"一键已读"时：

1. 先询问用户确认：列出将要标记为已读的普通邮件数量，并附按钮
2. 用户点击确认后，运行 `python3 ~/.ductor/workspace/skills/personal-assistant/mail_poll.py --mark-normal-read`
3. 把脚本输出结果回复给用户
4. **验证**：再次拉取邮件，确认普通邮件 UNSEEN 计数为 0。如不为 0，说明标记失败，需排查原因
