...
当收到回调 `mail:mark-normal-read`（用户点击按钮）或用户说"一键已读"时：

1. 运行 `python3 ~/.ductor/workspace/skills/personal-assistant/mail_poll.py --mark-normal-read`
2. 把输出结果回复给用户，并附上确认按钮：
   `[button:已读]`
